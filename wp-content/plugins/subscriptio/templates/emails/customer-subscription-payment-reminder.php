...
<p><?php printf(__('Your recent subscription renewal order on %s is due for payment.', 'subscriptio'), get_option('blogname')); ?></p>

<?php if ($next_action_is_overdue): ?>
    <p><?php printf(__('If you do not pay it by <strong>%s</strong>, your %s will be <strong>%s</strong> and will be subject to %s.', 'subscriptio'), $next_action_datetime, _n('subscription', 'subscriptions', count($order->get_items()), 'subscriptio'), $next_action, $subsequent_action); ?></p>
<?php else: ?>
