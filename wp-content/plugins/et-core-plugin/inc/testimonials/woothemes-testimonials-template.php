...
function woothemes_testimonials ( $args = '' ) {
	...
	do_action( 'woothemes_testimonials_before', $args );
		...
		if ( ! is_wp_error( $query ) && is_array( $query ) && count( $query ) > 0 ) {
			...
			$html .= '<div id="owl-testimonials" class="'.$class.' testimonials" data-navigation="'.$args['navigation'].'" data-auto_slide="'.$args['auto_slide'].'" data-interval="'.$args['interval'].'" >' . "\n";
			...
			foreach ( $query as $post ) { $count++;
				...
				if( $args['type'] == 'slider-grid' && ( $count - 1 ) % $per_slide == 0 ) {
					$html .= '<div class="testimonial-slider-item">';
				}
				...
				if( $args['type'] == 'slider-grid' && ( $count % $per_slide == 0 || $count == count($query) )) {
					$html .= '</div> <!--testimonial-slider-item-->';
				}
			...
			if ( $args['pagination'] == true && count( $query ) > 1 && $args['effect'] != 'none' ) {
				...
				$html .= '<a href="#" class="btn-prev">' . apply_filters( 'woothemes_testimonials_prev_btn', '&larr; ' . esc_html__( 'Previous', 'xstore' ) ) . '</a>' . "\n";
		        $html .= '<a href="#" class="btn-next">' . apply_filters( 'woothemes_testimonials_next_btn', esc_html__( 'Next', 'xstore' ) . ' &rarr;' ) . '</a>' . "\n";
		        $html .= '</div><!--/.pagination-->' . "\n";
			}
