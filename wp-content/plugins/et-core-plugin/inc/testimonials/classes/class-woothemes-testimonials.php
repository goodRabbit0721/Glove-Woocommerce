...
class Woothemes_Testimonials {
	...
	public function register_custom_column_headings ( $defaults ) {
		$new_columns = array( 'image' => esc_html__( 'Image', 'xstore' ) );
	...
	public function updated_messages ( $messages ) {
	  ...
	  $messages[$this->token] = array(
	    ...
	    1 => sprintf( esc_html__( 'Testimonial updated. %sView testimonial%s', 'xstore' ), '<a href="' . esc_url( get_permalink( $post_ID ) ) . '">', '</a>' ),
	    2 => esc_html__( 'Custom field updated.', 'xstore' ),
	    3 => esc_html__( 'Custom field deleted.', 'xstore' ),
	    4 => esc_html__( 'Testimonial updated.', 'xstore' ),
	    /* translators: %s: date and time of the revision */
	    5 => isset($_GET['revision']) ? sprintf( esc_html__( 'Testimonial restored to revision from %s', 'xstore' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
	    6 => sprintf( esc_html__( 'Testimonial published. %sView testimonial%s', 'xstore' ), '<a href="' . esc_url( get_permalink( $post_ID ) ) . '">', '</a>' ),
	    7 => esc_html__('Testimonial saved.', 'xstore'),
	    8 => sprintf( esc_html__( 'Testimonial submitted. %sPreview testimonial%s', 'xstore' ), '<a target="_blank" href="' . esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) . '">', '</a>' ),
	    9 => sprintf( esc_html__( 'Testimonial scheduled for: %1$s. %2$sPreview testimonial%3$s', 'xstore' ),
	      // translators: Publish box date format, see http://php.net/date
	      '<strong>' . date_i18n( esc_html__( 'M j, Y @ G:i', 'xstore' ), strtotime( $post->post_date ) ) . '</strong>', '<a target="_blank" href="' . esc_url( get_permalink($post_ID) ) . '">', '</a>' ),
	    10 => sprintf( esc_html__( 'Testimonial draft updated. %sPreview testimonial%s', 'xstore' ), '<a target="_blank" href="' . esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) . '">', '</a>' ),
	  );
	...
	public function meta_box_setup () {
		add_meta_box( 'testimonial-data', esc_html__( 'Testimonial Details', 'xstore' ), array( &$this, 'meta_box_content' ), $this->token, 'normal', 'high' );
	} // End meta_box_setup()
	...
	public function enter_title_here ( $title ) {
		if ( get_post_type() == $this->token ) {
			$title = esc_html__( 'Enter the customer\'s name here', 'xstore' );
		}
		return $title;
	...
	public function get_custom_fields_settings () {
		...
		$fields['gravatar_email'] = array(
		    'name' => esc_html__( 'Gravatar E-mail Address', 'xstore' ),
		    'description' => sprintf( esc_html__( 'Enter in an e-mail address, to use a %sGravatar%s, instead of using the "Featured Image".', 'xstore' ), '<a href="' . esc_url( 'http://gravatar.com/' ) . '" target="_blank">', '</a>' ),
		    'type' => 'text',
		    'default' => '',
		    'section' => 'info'
		);
		...
		$fields['byline'] = array(
		    'name' => esc_html__( 'Byline', 'xstore' ),
		    'description' => esc_html__( 'Enter a byline for the customer giving this testimonial (for example: "CEO of WooThemes").', 'xstore' ),
		    'type' => 'text',
		    'default' => '',
		    'section' => 'info'
		);
		...
		$fields['url'] = array(
		    'name' => esc_html__( 'URL', 'xstore' ),
		    'description' => esc_html__( 'Enter a URL that applies to this customer (for example: http://woothemes.com/).', 'xstore'  ),
		    'type' => 'url',
		    'default' => '',
		    'section' => 'info'
		);
