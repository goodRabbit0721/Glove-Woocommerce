...
extract( shortcode_atts( array(
	...
	'href' => '',
	'el_class' => '',
	'title' => esc_html__( 'Text on the button', 'xstore' ),
	'position' => ''
), $atts ) );
$a_class = '';
...
$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, ' ' . $color . $size . $icon . $el_class . $position, $this->settings['base'], $atts );

if ( $href != '' ) {
	$output .= '<span>' . $title . $i_icon . '</span>';
	$output = '<a class="btn ' . $a_class . $css_class . '" title="' . $title . '" href="' . $href . '"' . $target . '>' . $output . '</a>';
} else {
