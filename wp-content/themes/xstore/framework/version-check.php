...
class ETheme_Version_Check {
	...
	function __construct() {
		...
		$this->api_url = ETHEME_API . 'version/';
	...
	public function add_notice() {
		...
		if ( ! get_user_meta($user_id, $this->ignore_key) ) {
	        ...
	        printf('There is a new version %1$s of %2$s Theme available on <a href="%3$s" target="_blank">ThemeForest</a> | <a href="%4$s" target="_blank">View changelog.</a> | <a href="%5$s">Dismiss Notice</a>', $this->new_version, ETHEME_THEME_NAME, ETHEME_TF_LINK, ETHEME_CHANGELOG_LINK, '?' . $this->ignore_key . '=0');
	        echo "</p></div>";
		}
	}
