...
class ETheme_Recent_Comments_Widget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array('classname' => 'etheme_widget_recent_comments', 'description' => esc_html__( 'The most recent comments (Etheme edit)', 'xstore' ) );
        parent::__construct('etheme-recent-comments', '8theme - '.__('Recent Comments', 'xstore'), $widget_ops);
        ...
        if ( $comments ) {
            foreach ( (array) $comments as $comment) {
                ...
                $output .=  '<li class="recentcomments">';
                    ...
                    $output .=  ' '.__('by', 'xstore').' <span class="comment_author">'.get_comment_author_link().'</span>';
                $output .=  '</li>';
            }
        }
        $output .= '</ul>';
...
?>
        ...
        <p><label for="<?php echo $this->get_field_id('number'); ?>"><?php esc_html_e('Number of comments to show:', 'xstore'); ?></label>
