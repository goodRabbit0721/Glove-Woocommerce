...
class NMI_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$output .= '<div class="nmi-other-fields" style="display:none;">';

		$et_menu_fields = new ET_Menu_Fields();
		$output .= $et_menu_fields->menu_render_fields($item_id);
		
		// Append menu item upload link
		$output .= '</div><div class="nmi-upload-link nmi-div field-level-0 field-level-1 field-level-2" style="display: none;">';
		...
		if ( has_post_thumbnail( $item_id ) ) {
			...
			$remove_link = ' | <a href="#" data-id="' . $item_id . '" class="nmi_remove" onclick="NMIRemoveThumbnail(\'' . $ajax_nonce . '\',' . $item_id . ');return false;">' . esc_html__( 'Remove menu item image', 'xstore' ) . '</a>';
