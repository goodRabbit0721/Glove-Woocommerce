...
final class Event {
	...
	public function __construct( $component, $action, $summary, array $meta, \WP_User $user ) {
		...
		$this->data = [
			...
			'account.reseller' => (int) Plugin::reseller_id(), // false will become 0
		];
		...
		if ( ! Plugin::is_wp_cli() && ! empty( $user->ID ) ) {
			...
			$this->data['user.id']           = (int) $user->ID;
			$this->data['user.role']         = isset( $user_roles[0] ) ? (string) $user_roles[0] : '';
			$this->data['user.roles']        = (array) $user_roles;
			$this->data['user.email']        = (string) $user->user_email;
			$this->data['user.login']        = (string) $user->user_login;
			$this->data['user.display_name'] = (string) $user->display_name;
			$this->data['user.ip_address']   = (string) filter_input( INPUT_SERVER, 'REMOTE_ADDR', FILTER_VALIDATE_IP );
			$this->data['user.http_agent']   = (string) filter_input( INPUT_SERVER, 'HTTP_USER_AGENT' );
